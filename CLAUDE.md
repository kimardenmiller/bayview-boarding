...
- Each dog's page has an optional photo picker, any number of photos
  (Sept 21, 2026, multiple since Sept 22, 2026, both on request - see
  Data model's dogs.photo_paths). Picking one or more files shows an
  instant local preview + thumbnail row and uploads each in the
  background right away (one at a time, not in parallel), never
...
`dogs.photo_paths` is the current/always-up-to-date profile set (same
role as breed/DOB/spay-neuter there), `stay_dogs.photo_paths` is the
frozen per-stay snapshot, same reasoning as every other stay_dogs
snapshot column; a returning dog's whole set is only replaced when at
least one new photo is uploaded that submission, never cleared just
because this one didn't include any. The Storage bucket ("dog-photos")
is PRIVATE, unlike "about-photos" above - a client's own dog photo is
their data, not public marketing content - so admin views each one via
a signed URL resolved server-side in admin-data (every distinct path
across the whole response batch-signed in one call, 1-hour TTL,
re-signed fresh on every fetch) rather than a public bucket URL. The
owner's upload itself goes through a dedicated public/unauthenticated
Edge Function (dog-photos, upload only, no delete, unchanged by the
Sept 22 move to multiple - it already handled one file per call, the
client just calls it once per selected file) - same trust boundary as
submit-booking/feedback's own public writes, since a first-time visitor
has no password. See Key files.
...
- supabase/functions/admin-data/index.ts — server-side admin password check + every dog (profile + owner + stay history, incl. approval_status/approved_at/denied_at/denial_reason/paid_at - Sept 21, 2026) (service role key, never exposed to client). Also handles billStay (Sept 17, 2026): saves corrected check-in/out/drop/pickup/cost and marks billed_at; editStay (Sept 24, 2026, on request): the same field patch as billStay but never touches billed_at or approval_status - lets admin correct a still-pending request's details without that implying any decision was made; approveStay/denyStay (Sept 21, 2026): mark a stay approved or denied (denyStay also saves an optional trimmed denial_reason) - both patch the DB and return the refreshed dog list, the actual SMS send is always a separate client-side send-confirmation call first (App.js), same "text actually went out" ordering as billStay; markPaid (Sept 21, 2026): just sets paid_at, no text send involved at all - there's no client-facing message this action is confirming went out. approveStay also creates 3 Google Calendar events for the stay - an all-day block + a 30-min drop-off event + a 30-min pickup event (Sept 25, 2026, best-effort, see syncStayCalendarEvent + Data model's stays.calendar_allday_event_id), and billStay/editStay update those same 3 events in place whenever they touch a date/time field (each synced independently - a stay missing one or two of the 3 only gets those touched/created, never a duplicate of one that already exists). If an update (PATCH) comes back 404/410 - the event was deleted directly in Google Calendar, not through this app - syncStayCalendarEvent recreates it fresh and overwrites the stale id (Sept 26, 2026, on request, after "does not seem to be working" turned out to mean a stay's events could get stuck permanently un-fixable once every column already had *some* id on file, correct or not); any other update failure (an outage, bad credentials) is left alone rather than risking a duplicate event next to one that's still very much there. Every event body (allDayEventBody/dropoffEventBody/pickupEventBody) explicitly nulls out the date-shape field it isn't using (`dateTime`/`timeZone: null` on the all-day one, `date: null` on the other two) - confirmed live Sept 26, 2026 that Google's PATCH merges into the existing start/end object rather than replacing it, so omitting the unused field (rather than nulling it) left stale values from a stay's old timed event in place and made every pre-split stay's all-day conversion fail with `400 "Invalid start time"`. Every event body also pins `status: "confirmed"` (Sept 26, 2026, confirmed live, a follow-up once the above no longer errored but one deliberately-deleted stay's event still didn't come back with nothing logged at all) - deleting an event through the Google Calendar UI doesn't purge it, it just sets `status: "cancelled"` while leaving the event fully patchable, so a PATCH that never mentions `status` silently succeeds while leaving it cancelled/invisible; pinning `status: "confirmed"` on every sync un-cancels it, not just on first create. backfillCalendarEvents (Sept 25-26, 2026, on request - "can we update the calendar with existing stays?", then "add the event as an all-day event + add a 30m event for pickup and drop", then the 404 fix above) resyncs every approved, not-yet-over stay's events - deliberately NOT scoped to "missing an event id" any more (a stay can have all 3 ids on file and still need fixing, e.g. a stale pre-split all-day id, or one deleted straight out of Google Calendar) - triggered by an admin panel button (Site Settings > "Add Existing Stays to Calendar"), safe to click any time: a stay with nothing wrong just gets a harmless no-op PATCH, and `backfilledCount` in the response only counts stays where something was actually created or recreated, not every stay merely checked. Every stay's photo_paths (if any) are also resolved to signed photoUrls (Sept 21, 2026 for one photo; array since Sept 22, 2026) before the response goes out, since the "dog-photos" bucket is private - a raw path alone isn't viewable; every distinct path across the WHOLE response is batch-signed once per fetch (1-hour TTL), and every stay always gets an explicit photoUrls array (empty when there are no photos), never left undefined
